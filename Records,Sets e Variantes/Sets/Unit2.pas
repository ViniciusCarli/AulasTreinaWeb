...
type
  TDiasSemana = (Domingo, Segunda, Terça, Quarta, Quinta, Sexta, Sabado);
const
...
begin
  ...
  // DiaAtual in DiasExpediente then
    //ShowMessage('Hoje é dia de trabalhar')
  //else
    //ShowMessage('Hoje é dia de folga');

  if DiaAtual in DiasFolga then
    ShowMessage ('É um dia de folga')
  else if DiaAtual in DiasTrabalho then
    ShowMessage ('É um dia de trabalho normal')
  else
    ShowMessage ('É necessário escolher uma opção!');
end;
...
begin
  ...
  if NumeroEscolhido in MeuConjunto then
    ShowMessage('O numero escolhido está dentro do conjunto')
  else
    ShowMessage('O número está fora do conjunto');
